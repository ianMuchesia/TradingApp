  ...
  return (
	...
	<form  className="space-y-6 ng-untouched ng-pristine ng-valid" onSubmit={handleSubmit}>
		<div>
			...
			<input id="name" type="text" placeholder="" className="w-full p-3 border-2 rounded-xl dark:bg-gray-800" />
		</div>
		<div>
			<label htmlFor="email" className="text-sm">Email</label>
			<input id="email" type="email" className="w-full p-3 rounded-xl border-2 dark:bg-gray-800" />
		</div>
		<div>
			<label htmlFor="message" className="text-sm">Message</label>
			<textarea id="message" rows={3} className="w-full p-3 rounded-xl border-2 dark:bg-gray-800"></textarea>
		</div>
		<button
               style={{
                background:
