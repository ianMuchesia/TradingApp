...
const GraphData = ({ chartItem }: Props) => {
  ...
  const selectedDay = (timeFormat: string) => {
    if (timeFormat == "oneDay") {
      return `&resolution=${30}&from=${oneDay}&to=${currentTime}&token=${
        import.meta.env.VITE_FINNHUB_API_KEY
      }
      `;
    } else if (timeFormat == "oneWeek") {
      return `&resolution=${60}&from=${oneWeek}&to=${currentTime}&token=${
        import.meta.env.VITE_FINNHUB_API_KEY
      }
      `;
    } else {
      return `&resolution=W&from=${oneYear}&to=${currentTime}&token=${
        import.meta.env.VITE_FINNHUB_API_KEY
      }
      `;
    }
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `https://finnhub.io/api/v1/stock/candle?symbol=${chartItem}${selectedDay(
            timeFormat
          )}`
        );
